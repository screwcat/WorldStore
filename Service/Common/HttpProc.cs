...
namespace Service.Common
{
    ...
    public class WebClient
    {
        ...
        public string GetHtmlUTF8(string url)
        {
            ...
            HttpWebRequest request = CreateRequest(url, "GET");
            respHtml = en.GetString(GetData(request));
            return respHtml;
        }
        ...
        public string GetHTML(string url) 
        {
            ...
            using (StreamReader read = new StreamReader(resp.GetResponseStream(), enc))
            {
                ...
                Match charSetMatch = Regex.Match(sHTML, "charset=[\'|\"]{0,1}.*?[\'|\"|\\s]{1}", RegexOptions.IgnoreCase);
                string sChartSet = charSetMatch.ToString().Substring(0, charSetMatch.ToString().Length - 1);
                string cat = sChartSet.Split('=')[1];
                //if it's not utf-8,we should redecode the html.  
                //if (!string.IsNullOrEmpty(cat) && !sChartSet.Equals("utf-8", StringComparison.OrdinalIgnoreCase))
                //{
                //    enc = Encoding.GetEncoding(cat);
        ...
        public string GetHtmlUTF8(string url, string referer)
        {
            Encoding en = Encoding.UTF8;
            HttpWebRequest request = CreateRequest(url, "GET");
            request.Referer = referer;
        ...
        /// <returns></returns>
        public byte[] GetData(string url, ref string contentType) 
        {
            HttpWebRequest request = CreateRequest(url, "GET");
            return GetData(request, ref contentType);
        }
        /// <summary>    
        /// 向指定URL发送文本数据    
        /// </summary>    
        ...
        /// <param name="request">请求对象</param>    
        /// <returns></returns>    
        private byte[] GetData(HttpWebRequest request, ref string contentType)
        {
            HttpWebResponse response;
            try
            {
